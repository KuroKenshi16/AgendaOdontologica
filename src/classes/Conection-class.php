...
class DB 
{
    ...
    public function allLike($query,  $requestData, $colunsSearch)
    {
        if (!$this->bConnected) {
            $this->Connect();
        }
        ...
        $filtro = $requestData['FIND'];
        $c = count($colunsSearch);
        if( !empty( $filtro ) ){
            for($i=0; $i < $c; $i++){
                $coluna = $colunsSearch[$i];
                if($i == 0){
                    $query .= "AND $coluna LIKE '$filtro%' ";
                }else{
                    $query .= "OR $coluna  LIKE '$filtro%' ";
                }
            }
        }
    ...
    public function listDataTables($query,  $requestData, $colunsSearch)
    {
        if (!$this->bConnected) {
            $this->Connect();
        }
        ...
        $query .= " ORDER BY $ordem $direcao LIMIT $inicio, $tamanho ";
        $resultado = $this->pdo->query($query);
        $qtdeLinhas = $resultado->rowCount();
